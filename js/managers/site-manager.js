...
export const SiteManager = (() => {
    ...
    function createCornerMarkers(width, length) {
        ...
        // Collect objects to remove to avoid modifying the scene graph while traversing it
        const toRemove = [];
        scene.traverse(child => {
            if (child.userData?.isCornerMarker || child.userData?.isCornerLabel) {
                toRemove.push(child);
            }
        });
        ...
        toRemove.forEach(child => {
            ...
            if (child.material) {
                // Special care for sprite materials that have a texture map
    ...
    // Create corner markers
     function createCornerMarkersxx(width, length) {
        const scene = Engine.getScene();
        if (!scene) return;
